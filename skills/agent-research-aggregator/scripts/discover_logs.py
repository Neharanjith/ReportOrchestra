...
def main():
    ...
    parser.add_argument("--max-files", type=int, default=0,
                        help="Stop discovery after finding this many files (0 = unlimited)")
    parser.add_argument("--quick", action="store_true",
                        help="Only scan priority directories and HIGH-priority patterns, skip general file scan")
    ...
    def _check_max() -> bool:
        return max_files is not None and len(all_entries) >= max_files
    ...
    for root in search_roots:
        ...
        for agent, spec in AGENT_SPECS.items():
            ...
            for base in dirs_to_scan:
                ...
                if _check_max():
                    break
                # In quick mode, only scan the first (priority) pattern group
                if args.quick:
                    break
        ...
        # --- General project file scan (skipped in --quick mode) ---
        if not args.quick:
