...
PHASE 1 — Parallel Candidate Discovery
   ...
   For each limitation_search_query in each related_work cluster:
     - Use your web search tool to discover up to ~10 candidate papers.
...
You do the LLM/web work; the deterministic helpers in `scripts/`
do the math.
...
Literature Review Agent prompt at `references/prompt.md`.
...
| `authors_note.md` | contents of optional `authors_note.md`; empty if absent |
...
These are excerpted from `references/prompt.md`. You MUST honor
