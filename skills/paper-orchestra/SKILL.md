...
`SKILL.md` and `references/` directories — you will load them
as you go.
...
## Inputs (the (I, E, T, G, F) tuple from the paper, plus optional author guidance)
...
| `workspace/inputs/authors_note.md` | — | no | Author preferences and global constraints used by outlining, section writing, and refinement |
...
If `workspace/inputs/authors_note.md` exists and is non-empty, the Outline
Agent MUST read it in full and use it as guidance when constructing the
outline. Conference requirements, supplied evidence, citation-verification
rules, and scientific integrity take precedence over conflicting author
guidance. If the file is absent or empty, preserve the original pipeline
behavior.

The Section Writing Agent and Content Refinement Agent MUST also read a
non-empty author note directly. Plotting and literature-search decisions flow
through the outline, while global constraints such as the exact title,
anonymity, and framing must remain in force during paper assembly and revision.
...
**Halt if validation fails** — every downstream step depends on the schema.
...
- `authors_note.md` (if present and non-empty; preserve its compatible global
...
If any gate fails, you must fix the issue (re-prompting the writing
...
otherwise you compute hashes inline).
...
│   ├── template.tex
│   ├── conference_guidelines.md
│   ├── authors_note.md              # optional guidance for outlining, writing, and refinement
