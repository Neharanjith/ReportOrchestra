...
### `authors_note.md` — Author Guidance

Optional Markdown containing author preferences or constraints for the paper,
such as an exact title, desired framing, anonymity requirements, or topics to
emphasize. When the file exists and is non-empty, the Outline Agent reads it
in full and incorporates compatible guidance into `outline.json`. The Section
Writing Agent and Content Refinement Agent also read the note directly so that
global constraints—especially an exact title, anonymity requirements, and
framing preferences—are preserved when assembling and revising the paper.

Conference requirements, the supplied experimental evidence, verified
citations, and scientific integrity take precedence over conflicting notes.
The file is advisory: its absence or an empty file preserves the original
PaperOrchestra behavior and must not fail input validation. Guidance that is
specific to plotting or literature search flows through the validated outline;
global paper constraints are additionally enforced from the note by downstream
writing and refinement agents.
