...
explicitly designs it as one comprehensive call so your LLM can maintain
...
- `workspace/inputs/authors_note.md` — optional author preferences and global
  constraints. If absent or empty, preserve the original behavior.
...
Author guidance applies only when compatible with conference requirements,
supplied evidence, verified citations, and scientific integrity. It must never
be used to invent results or references. Unlike plotting and literature-search
preferences, which flow through `outline.json`, global constraints such as an
exact title or anonymous byline are applied directly at this assembly stage.

If your host LLM has no vision input, fall back to text-only mode: pass the
captions in `captions.json` as descriptions and tell your LLM it cannot see
...
These are excerpted from `references/prompt.md` (App. F.1, pp. 47-49). You
MUST honor them on the writing call:
