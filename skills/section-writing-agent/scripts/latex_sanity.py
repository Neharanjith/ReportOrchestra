...
def strip_latex_comments(text: str) -> str:
    r"""Remove LaTeX comments while preserving escaped percent signs.

    A percent sign starts a comment only when preceded by an even number of
    consecutive backslashes. With an odd number, the final backslash escapes
    the percent sign (for example, ``54\%``).
    """
    output: list[str] = []
